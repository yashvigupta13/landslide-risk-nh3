...
# Only published input file — provides Lat/Long for the AOI extent below.

print("\n===== CLIP TO REGION (WITH SAFETY BUFFER) =====\n")

BASE_CSV     = "processed1_truncated_data_with_rainfall_jenks.csv"
CSV_POINTS   = BASE_CSV
...
CSV_OUT = "extracted_avg_rainfall_values.csv"   # intermediate file, not a repo input
...
# df_proc/df_rain both come from BASE_CSV, same row order by construction.
# BASE_CSV ships with a MAX-based Rainfall_Jenks column — drop it below and
# replace with an AVG-based one. Written to a new filename so the published
# MAX demo file isn't overwritten; rename FINAL_CSV to use it downstream.

CSV_PROCESSED = BASE_CSV
CSV_RAIN      = CSV_OUT   # extracted_avg_rainfall_values.csv, written above
FINAL_CSV     = "processed1_truncated_data_with_rainfall_jenks_AVG.csv"
...
if "Rainfall_Jenks" in df_proc.columns:
    df_proc = df_proc.drop(columns=["Rainfall_Jenks"])  # drop MAX-based column

rainfall = df_rain[RAIN_COL].values

breaks = jenkspy.jenks_breaks(rainfall, n_classes=N_CLASSES)

print("\nJenks break values (AVERAGE rainfall):")
...
df_proc.to_csv(FINAL_CSV, index=False)
print("\n✓ Jenks rainfall classification completed (AVERAGE-rainfall variant)")
print("✓ Output written to:", FINAL_CSV)
...
CSV_IN = FINAL_CSV   # AVG-rainfall variant written above
...
CSV_IN  = FINAL_CSV   # same AVG-rainfall file used for undersampling above
